...
class RuntimeJobV2(BasePrimitiveJob[PrimitiveResult, JobStatus], BaseRuntimeJob):
    ...
    def result(
        ...
        If the decoded job results are of type QuantumProgramResult, and if a post processor is
        specified (explicitly by the users, or via the results' passthrough data), the results are
        post-processed before being returned. If provided, the user-specified `post_processor`
        takes precedence over the post processor specified in the passthrough data.
